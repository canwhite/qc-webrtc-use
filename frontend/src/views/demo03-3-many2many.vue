...
<script>
	...
	/** 
		A: 进入房间，发现没有，没有创建RTC链接,但是获取了getUserMedia()到video.srcObject上,但是屏幕上摄像头和音视频是默认关闭的
		B：进入房间，发现有A, 于是B创建RTC链接，命名为B-A；创建offer，触发offer事件，A监听到offer事件，于是也开始创建RTC，命名A-B, 然后创建answer，触发answer事件，B收到anwser事件保存answer，至此 A B的链接建立；

		其实就是一个RTC链接，只不过在A,B里面叫法不一样，在A中叫A-B 在B中叫B-A;

		C: 进入房间，依次和A、B简历链接，并且有各自的命名；如图；

		至此 多对多的会议就建立了；建立连接后，各自维护的链接情况:

		A: A-B A-C
		B: B-A B-C
		C: C-A C-B

		其中A-B和B-A 是同一个 A-C和C-A是同一个 B-C和C-B是同一个[捂脸] 应该是这个关系
	 * 
	 * 
	 */
